...
interface PhoneNumberUploadProps {
  onNumbersExtracted: (numbers: string[], file: File) => void;
  disabled?: boolean;
  currentFile?: File | null;
}

export default function PhoneNumberUpload({
  onNumbersExtracted,
  disabled = false,
  currentFile = null,
}: PhoneNumberUploadProps) {
  ...
  const handleFileSelect = useCallback(
    async (file: File) => {
      ...
      try {
        const numbers = await parsePhoneNumbers(file);
        ...
        if (numbers.length > 1000) {
          setError("Maximum 1000 phone numbers allowed per bulk message");
          return;
        }
    ...
    <div className="space-y-3">
      {!currentFile ? (
        <div
          ...
          onDrop={handleDrop}
          onClick={() =>
            !disabled && document.getElementById("phone-file-input")?.click()
          }
        >
          <input
            id="phone-file-input"
            ...
            aria-label="Phone number file upload"
          />
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <p className="text-xs text-github-fg-subtle mt-1">
                Maximum 1000 phone numbers, 5MB file size
              </p>
            </div>
