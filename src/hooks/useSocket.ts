...
export const useSocket = (): UseSocketResult => {
  ...
  useEffect(() => {
    ...
    newSocket.on("session:authenticated", (data: { sessionId: string }) => {
    ...
    newSocket.on("session:ready", (data: { sessionId: string; clientInfo: { pushname: string; wid: string; platform: string } }) => {
    ...
    newSocket.on("message:received", (data: { sessionId: string; message: IncomingMessage }) => {
    ...
    newSocket.on("message:sent", (data: { sessionId: string; messageId: string; to: string; message: string }) => {
    ...
    newSocket.on("message:ack", (data: { sessionId: string; messageId: string; ack: number; status: string }) => {
    ...
    newSocket.on("typing:status", (data: { sessionId: string; from: string; status: string }) => {
    ...
    newSocket.on("message:revoked", (data: { sessionId: string; messageId: string; from: string }) => {
    ...
    newSocket.on("presence:update", (data: { sessionId: string; id: string; presence: { id: string; lastSeen: number | null; isOnline: boolean; isUser: boolean } }) => {
      console.log(
        "👤 Presence update:",
        data.id,
    ...
    const loadSessions = async () => {
      try {
        ...
        newSocket.emit("sessions:list", (response: SocketResponse<WhatsAppSessionSocket[]>) => {
