...
export interface BulkMessageWithPersonalizationRequest {
  message: string;
  data: Array<{
    [key: string]: string;
  }>;
...
export const whatsappApi = {
  ...
  async sendBulkTextMessage(
    ...
    const response = await api.post(`/sessions/${sessionId}/send-bulk-text`, data);
    return response.data;
  },

  async sendBulkTextMessageWithPersonalization(
    sessionId: string,
    data: BulkMessageWithPersonalizationRequest
  ): Promise<ApiResponse<{ jobId: string }>> {
    const response = await api.post(`/sessions/${sessionId}/send-personalized-bulk-text`, data);
    return response.data;
  },



  async getBulkJob(jobId: string): Promise<ApiResponse<BulkJob>> {
    const response = await api.get(`/bulk-jobs/${jobId}`);
    return response.data;
  },

  async getAllBulkJobs(): Promise<ApiResponse<BulkJob[]>> {
